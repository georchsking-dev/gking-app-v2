...
export const openAIAccessSchema = z.object({
  dialect: z.enum([
    ...
    'openrouter', 'perplexity', 'sakanaai', 'togetherai', 'xai', 'zai', 'huggingface',
  ]),
...
export function openAIAccess(access: OpenAIAccessSchema, modelRefId: string | null, apiPath: string): { headers: HeadersInit, url: string } {
  switch (access.dialect) {
    ...
    case 'zai':
      ...
      let zaiKey = access.oaiKey || env.ZAI_API_KEY || '';
      const zaiHost = llmsFixupHost(access.oaiHost || env.ZAI_API_HOST || 'https://zenmux.ai/api/v1', apiPath);
      const zaiPath = zaiHost.includes('zenmux.ai') ? apiPath : apiPath.replace('/v1', '/v4');
    ...
    case 'huggingface':
      let hfKey = access.oaiKey || env.HUGGINGFACE_API_KEY || '';
      const hfHost = llmsFixupHost(access.oaiHost || 'https://router.huggingface.co/openai', apiPath);

      hfKey = llmsRandomKeyFromMultiKey(hfKey);

      if (!hfKey)
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Missing Hugging Face API Key. Add it on the UI (Models Setup) or server side (your deployment).' });
